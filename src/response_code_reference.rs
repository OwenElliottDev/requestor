pub fn http_status_meaning(code: u16) -> &'static str {
    match code {
        // 1xx - Informational
        100 => {
        ...
        207 => {
            "206 Multi-Status (WebDAV) - Conveys information about multiple resources, for situations where multiple status codes might be appropriate."
        }
        208 => {
            "206 Already Reported (WebDAV) - Used inside a <dav:propstat> response element to avoid repeatedly enumerating the internal members of multiple bindings to the same collection."
        }
        226 => {
            "206 IM Used (HTTP Delta encoding) - The server has fulfilled a GET request for the resource, and the response is a representation of the result of one or more instance-manipulations applied to the current instance."
        }
        ...
        // Unknown or not standard
        _ => {
            "Unknown or unassigned status code - The code is not part of the standard HTTP status codes or is not commonly used."
        }
    }
}
